<?php
/**
 * Product spec header — featured image, category eyebrow, title, spec
 * string, doc code. Reads the current product's post meta directly at
 * render time rather than via Elementor Dynamic Tags: it's a real
 * Addlar_Base_Widget, so it gets the theme's `.adl` scope wrapper (see
 * open_section()) the same way every homepage widget does — Elementor's
 * native widgets don't, which is why the first cut of this template
 * rendered unstyled.
 *
 * @package Addlar
 */
	...
	protected function render() {
		...
		<div class="wrap" style="max-width:860px;">
			...
			<?php if ( 'yes' === $s['show_image'] && has_post_thumbnail( $post_id ) ) : ?>
				<div class="imgwrap" style="margin-bottom:24px;overflow:hidden;">
			...
			<?php if ( $doc ) : ?>
				<p class="spec-table-note"><?php echo esc_html( $doc ); ?></p>
			<?php endif; ?>
